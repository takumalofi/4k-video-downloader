...
def cookie_icon(size=18, color=None):
    ...
    fill.setAlpha(30)
    body = QPainterPath()
    body.addEllipse(QPointF(9.0, 9.6), 5.9, 5.9)
    bite = QPainterPath()
    bite.addEllipse(QPointF(13.9, 4.6), 3.1, 3.1)
    shape = body.subtracted(bite)
    p.fillPath(shape, QBrush(fill))
    p.strokePath(shape, _pen(color, 1.5))
    p.setPen(Qt.NoPen)
    p.setBrush(QBrush(color))
    p.drawEllipse(QPointF(6.4, 7.6), 1.0, 1.0)
    p.drawEllipse(QPointF(9.6, 10.8), 1.2, 1.2)
    p.drawEllipse(QPointF(6.9, 11.9), 0.8, 0.8)
    p.end()
    return QIcon(pm)
