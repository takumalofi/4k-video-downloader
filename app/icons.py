...
def cookie_icon(size=18, color=None):
    ...
    fill.setAlpha(30)
    body = QPainterPath()
    body.addEllipse(QPointF(8.8, 9.8), 7.0, 7.0)
    bite = QPainterPath()
    bite.addEllipse(QPointF(14.6, 3.6), 3.6, 3.6)
    shape = body.subtracted(bite)
    p.fillPath(shape, QBrush(fill))
    p.strokePath(shape, _pen(color, 1.6))
    p.setPen(Qt.NoPen)
    p.setBrush(QBrush(color))
    p.drawEllipse(QPointF(5.2, 7.6), 1.3, 1.3)
    p.drawEllipse(QPointF(9.6, 11.4), 1.5, 1.5)
    p.drawEllipse(QPointF(5.6, 12.6), 1.0, 1.0)
    p.drawEllipse(QPointF(11.9, 8.6), 0.8, 0.8)
    p.end()
    return QIcon(pm)
